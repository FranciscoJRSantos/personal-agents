...
## Step 1: Find the Experiment Artifact

```bash
TICKET=$(git branch --show-current | grep -oE '[A-Z]+-[0-9]+')
ls .claude/artifacts/${TICKET}-experiment-*.md 2>/dev/null | sort -r | head -5
```
...
```bash
ls .claude/artifacts/experiment-*.md 2>/dev/null | sort -r | head -5
```
