...
Call the `test_writer` agent with the plan + diff as input:

```bash
(cat .claude/artifacts/${TICKET}-plan.md 2>/dev/null; git diff main...HEAD) | test_writer
```

Or if no plan artifact:

```bash
git diff main...HEAD | test_writer
```

The agent outputs test code following the project's testing conventions. Review the output
before writing to disk — correct any test that uses wrong imports, wrong fixtures, or
fabricates implementation details.
...
Write `.claude/artifacts/<TICKET>-tests.md`:
