...
For each changed file, read the full file (not just the diff) if needed to understand surrounding context — especially for logic bugs, architectural issues, or missing error handling.

Use judgment: small style issues don't need full file context, but anything touching business logic or data flow does.

---

## Step 5: Perform the Review

Apply the base review rules below, **plus** any project-level rules from `.claude/review.md`.
...
Write `.claude/artifacts/<LABEL>-review-impl.md`:
...
## Project-Level Skill Format (for `.claude/review.md`)

Users can create a `.claude/review.md` file in their repo root to customize the review for that project. Example:
