...
- `nb_check` failures should be reported as WARN (not FAIL) — notebooks are non-blocking for most merge workflows in this team.
...
- For flaky tests (random seed, network timeout), note the non-deterministic pattern and suggest a re-run before treating it as a real failure.
