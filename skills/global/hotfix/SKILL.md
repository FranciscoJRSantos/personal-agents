...
- Do not use `git add -A` — it can accidentally stage `.env` files, generated artifacts, or large binaries. Review `git status` first and stage only the changed files.
- If the change exceeds ~100 lines, suggest the full `/review` + `/ship` flow instead — hotfix is for small, targeted changes only.
- Never skip checks (`--no-verify`) even under time pressure. A broken hotfix is worse than a delayed one.
- `glab mr create` requires the branch to already be pushed — push before creating the MR.
...
Show the proposed message and staged files. Review `git status` first, then stage specific files:

```bash
git status
git add <specific files changed by the fix>
git diff --staged --stat
```

Ask user to confirm staged files before committing. Then commit:
