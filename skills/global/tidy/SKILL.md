...
name: tidy
description: >
  Clean up stale artifacts from .claude/artifacts/ for tickets that are done,
...
Identify and remove stale artifacts from `.claude/artifacts/` for completed work.
...
```bash
ls -la .claude/artifacts/*.md 2>/dev/null
```
...
```bash
ls .claude/artifacts/*.md 2>/dev/null | grep -oE '[A-Z]+-[0-9]+' | sort -u
```
...
- Offer to archive instead of delete if the user prefers (`mv` to `.claude/artifacts/.archive/`)
