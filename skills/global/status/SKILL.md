...
If not on a feature branch (e.g. on `main` or `develop`), skip to Step 5 (No Active Work).
...
LABEL=${TICKET:-$BRANCH}
echo "=== Artifacts ==="
for f in .claude/artifacts/${LABEL}-*.md; do
  [ -f "$f" ] || continue
  name=$(basename "$f")
...
2. **Plan exists, zero commits** ahead of main → "Plan is ready — start implementing."
...
## Step 5: No Active Work (on main / develop)
